...
class DataPreprocessing:
    ...
    def preprocess_function(self, dataset):
        ...
        model_inputs = tokenizer(inputs, max_length=self.config.inputs_max_length, padding='max_length', truncation=True)
        try:
            labels = tokenizer(targets, max_length=self.config.labels_max_length, padding='max_length', truncation=True)
    ...
    def preprocess_data(self, train_data, test_data):
        tokenized_train_data = train_data.map(self.preprocess_function, batched=True, remove_columns=train_data.column_names)
        tokenized_test_data = test_data.map(self.preprocess_function, batched=True, remove_columns=test_data.column_names)
